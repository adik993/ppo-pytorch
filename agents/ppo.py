...
class PPOLoss(_Loss):
    ...
    def __init__(self, clip_range: float, c_entropy: float, c_value: float):
        """
    ...
    def forward(self, distribution_old: Distribution, value_old: Tensor, distribution: Distribution,
        ...
        # Value loss
        value_old_clipped = value_old + (value - value_old).clamp(-self.clip_range, self.clip_range)
...
class PPO(Agent):
    ...
    def __init__(self, env: MultiEnv, model_factory: ModelFactory, reward: Reward, advantage: Advantage,
                 learning_rate: float, clip_range: float, c_entropy: float, c_value: float,
                 n_mini_batches: int, n_optimization_epochs: int, clip_grad_norm: float) -> None:
        """
        ...
        """
        super().__init__(env, model_factory)
        self.reward = reward
        ...
        self.loss = PPOLoss(clip_range, c_entropy, c_value)
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, rewards: np.ndarray, dones: np.ndarray):
        ...
        dataset = self.model.dataset(policy_old[:, :-1], values_old[:, :-1], states[:, :-1], actions,
        ...
        with torch.autograd.detect_anomaly():
            for _ in range(self.n_optimization_epochs):
                for tuple_of_batches in loader:
                    batch_policy_old, batch_values_old, batch_states, batch_actions, batch_rewards, batch_advantages = \
...
if __name__ == '__main__':
    ...
    agent = PPO(MultiEnv('CartPole-v1', 4),
                MLP.factory(),
                GeneralizedRewardEstimation(gamma=0.99, lam=0.95),
                GeneralizedAdvantageEstimation(gamma=0.99, lam=0.95),
                learning_rate=5e-3,
                clip_range=0.2,
                c_entropy=1e-2,
                c_value=0.5,
    ...
    agent.learn(epochs=1, n_steps=500, render=True)
